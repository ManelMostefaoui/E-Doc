...
const PatientsManagement = () => {
  ...
  const handlePatientSelect = (patient) => {
    ...
    if (!patient) {
      setError("Cannot view patient details: No patient provided");
      return;
    }
    
    if (!patient.id) {
      console.error("Cannot view patient details: Patient ID is missing", patient);
      setError("Cannot view patient details: Patient ID is missing");
      return;
    }
    
    try {
      sessionStorage.setItem('tempPatient_' + patient.id, JSON.stringify(patient));
      console.log('Successfully stored patient in sessionStorage before navigation:', patient.id);
      
      navigate(`/patients/${patient.id}`);
    } catch (error) {
      console.error('Error saving patient data before navigation:', error);
      navigate(`/patients/${patient.id}`);
    }
  };
