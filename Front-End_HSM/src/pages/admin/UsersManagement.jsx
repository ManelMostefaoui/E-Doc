...
import AddUserModal from "../../components/CAdmin/AddUserModal";
  ...
  const handleDeleteUsers = async (selectedUsers) => {
    ...
    try {
      ...
      for (const user of selectedUsers) {
        try {
          let userId = user.id;
          // Only skip backend deletion if the id is a generated one (client-side only)
          ...
          if (err.response) {
            ...
            if (err.response.data && err.response.data.message) {
              ...
              if (errorMessage.includes('foreign key constraint fails')) {
                errorMessage = `Cannot delete user ${user.name} because they have associated appointments. The system will attempt to delete these appointments first.`;
              }
            }
          }
      ...
      // After attempting all deletions, refresh the user list from the backend
      await fetchUsers();
      ...
      // Show summary of results with more detailed error messages
      if (results.successful.length > 0 && results.failed.length === 0) {
        ...
        const failedUsersList = results.failed.map(f => `\n- ${f.user.name}: ${f.error}`).join('');
        alert(`Successfully deleted ${results.successful.length} user(s), but failed to delete ${results.failed.length}:\n${failedUsersList}`);
      } else if (results.failed.length > 0) {
        const failedUsersList = results.failed.map(f => `\n- ${f.user.name}: ${f.error}`).join('');
        alert(`Failed to delete ${results.failed.length} user(s):\n${failedUsersList}`);
      }

    } catch (err) {
      console.error("Error during batch user deletion process:", err);

      if (err.response && err.response.status === 401) {
        localStorage.removeItem('token');
        navigate('/login');
      } else {
        setError("An unexpected error occurred while processing deletions. Please refresh and try again.");
      }
    } finally {
      setLoading(false);
    }
